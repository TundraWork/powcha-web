...
test('should be without the logo', async ({ page }) => {
  await page.goto('./e2e/index.html');
  expect(await page.locator('#test-hidelogo .powcha-logo').first().count()).toBe(0);
});

test('should be without the footer', async ({ page }) => {
  await page.goto('./e2e/index.html');
  expect(await page.locator('#test-hidefooter .powcha-footer').first().count()).toBe(0);
});

test('should show expired error', async ({ page }) => {
  await page.goto('./e2e/index.html');
  await delay(100);
  expect(await page.locator('#test-expire .powcha-error').first().count()).toBe(0);
  await delay(2000);
  expect(await page.locator('#test-expire .powcha-error').first().count()).toBe(1);
...
test('should set options via configure()', async ({ page }) => {
  ...
  expect(await page.locator('#test-configure .powcha-logo').first().count()).toBe(0);
});
